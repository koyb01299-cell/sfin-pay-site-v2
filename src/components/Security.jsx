...
export default function Security() {
  return (
    <section
      id="security"
      className="relative py-28 px-6 md:px-16 bg-gradient-to-b from-[#f8f7ff] to-[#ebe9f7]"
    >
      {/* ✨ 헤더 */}
      <motion.div
        {...fadeUp(0)}
      ...
      >
        ...
        <h2 className="mt-6 text-4xl md:text-5xl font-extrabold text-[#1f1631] leading-tight">
          금융 보안 수준의 <span className="text-[#7c3aed]">데이터 보호 체계</span>
        </h2>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {[
          ...
          <motion.div
            key={i}
            {...fadeUp(i + 1)}
            className="group relative rounded-2xl border border-[#e2d6ff]/60 bg-white p-6 hover:shadow-[0_10px_30px_rgba(124,58,237,0.15)] transition-all duration-500"
          >
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-[#4b3a6b]/80 leading-relaxed">
                  {f.desc}
                </p>
              </div>
            </div>
      ...
      >
        <h3 className="text-2xl font-bold text-[#2d1b55] mb-10">
          SFIN PAY의 <span className="text-[#7c3aed]">보안 계층 구조</span>
        </h3>
        <div className="flex flex-col md:flex-row items-center justify-center gap-6">
          {[
            ...
            <motion.div
              ...
              className="flex flex-col items-center gap-3 bg-white p-5 rounded-xl border border-[#e2d6ff] shadow-[0_6px_20px_rgba(124,58,237,0.1)] w-[180px]"
            >
              <div className="p-3 rounded-full bg-[#8b5cf6]/10 text-[#7c3aed]">
                {layer.icon}
              </div>
              <p className="text-sm font-medium text-[#2d1b55]">
                {layer.label}
              </p>
            </motion.div>
          ))}
      ...
      <motion.div
        {...fadeUp(8)}
        className="max-w-5xl mx-auto text-center rounded-2xl border border-[#e2d6ff] bg-white p-10 shadow-[0_6px_25px_rgba(124,58,237,0.08)]"
      >
        <p className="text-[#4b3a6b]/80 text-lg mb-4 leading-relaxed">
          모든 결제 데이터는 암호화되어 저장되며,
          API 및 접근 로그는 5년 이상 안전하게 보존됩니다.
          SFIN PAY는 금융 보안 표준을 넘어서는 <b>투명한 보안 체계</b>를 제공합니다.
        </p>
